...
//        int x = (args.length > 0) ? 1 : 2;
        ...
        try {
            pst = con.prepareStatement("UPDATE "+db+".UserPermission SET AddProduct=?,AddSupplyer=?,AddBrand=?,AddCatagory=?,AddUnit=?,AddCustomer=?,UpdateProduct=?,UpdateSupplyer=?,UpdateBrand=?,UpdateCatagory=?,UpdateUnit=?,UpdateCustomer=?,SellProduct=?,EmployeManage=?,ChangeOwnPass=? WHERE UserId=?");
//            pst.setString(1, id);
            ...
            pst.setInt(14, EmployeManage);
            pst.setInt(15, ChangeOwnPass);
            pst.setString(16, id);
