    ...
    public void view(CurrentProduct currentProduct) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                currentProduct.userId = rs.getString(12);
                currentProduct.date = rs.getString(13);
                ...
                currentProduct.currentProductList.addAll(new ListProduct(currentProduct.id, currentProduct.productId, currentProduct.productName, currentProduct.quantity, currentProduct.description, currentProduct.supplierName, currentProduct.brandName, currentProduct.catagoryName, currentProduct.unitName, currentProduct.pursesPrice, currentProduct.sellPrice, currentProduct.userName, currentProduct.date));
            }
            pst.close();
            con.close();
            rs.close();
    ...
    public void update(CurrentProduct currentProduct) {
        ...
        try {
            pst = con.prepareStatement("update "+db+".Products set ProductId=?, ProductName=?, Quantity=?, Description=?, "
                    ...
                    + " UnitId=?, PursesPrice=?, SellPrice=?, Date=?  where Id=?");
            pst.setString(1, currentProduct.productId);
            ...
            pst.setString(12, currentProduct.id);
            pst.executeUpdate();
            pst.close();
            con.close();
