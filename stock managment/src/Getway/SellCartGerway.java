...
//            Dialogs.create().title(null).masthead("Soled").message("Soled Sucessfuly").styleClass(Dialog.STYLE_CLASS_UNDECORATED).showInformation();
    ...
    public void update(SellCart sellCart){
        con = dbCon.geConnection();
        try {
            pst = con.prepareStatement("update "+db+".Sell set SellPrice=?, Quantity=?, TotalPrice=? where Id=?");
            pst.setString(1, sellCart.sellPrice);
            pst.setString(2, sellCart.quantity);
            pst.setString(3, sellCart.totalPrice);
            pst.setString(4, sellCart.Id);
            pst.executeUpdate();
            pst.close();
            con.close();
    ...
    public void view(SellCart sellCart){
        ...
        try {
            ...
            while (rs.next()){
                ...
                sellCart.sellerID = rs.getString(9);
                sellCart.sellDate = rs.getString(10);
                ...
                sellCart.soldList.addAll(new ListSold(sellCart.Id,sellCart.sellID ,sellCart.productID, sellCart.givenProductID, sellCart.customerID, sellCart.customerName, sellCart.pursesPrice, sellCart.sellPrice, null, sellCart.quantity, sellCart.totalPrice, sellCart.pursrsDate, sellCart.sellerID, sellCart.sellerName, sellCart.sellDate));
            }pst.close();
            con.close();
            rs.close();
        } catch (SQLException ex) {
    ...
    public void selectView(SellCart sellCart){
        con = dbCon.geConnection();
        SQL sql = new SQL();
        try {
            pst = con.prepareStatement("select * from "+db+".Sell where id=?");
            pst.setString(1, sellCart.Id);
            rs = pst.executeQuery();
            while (rs.next()){
                sellCart.sellID = rs.getString(2);
                sellCart.customerID = rs.getString(3);
